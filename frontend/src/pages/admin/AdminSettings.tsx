...
export default function AdminSettings() {
  ...
  return (
    <div className="max-w-6xl mx-auto flex flex-col md:flex-row gap-8 animate-fade-in min-h-[800px]">
      ...
      <aside className="w-full md:w-72 flex flex-col gap-3">
        <div className="px-4 mb-4">
           ...
           <h2 className="text-xl font-black text-white uppercase italic">Neural <span className="text-emerald-500 not-italic">Core</span></h2>
        </div>
        ...
        <nav className="flex flex-col gap-2">
          {tabs.map(tab => (
            <button
              ...
              className={`group flex items-center gap-4 p-5 rounded-[2rem] border transition-all text-left relative overflow-hidden ${
                activeTab === tab.id 
            ...
            >
              {activeTab === tab.id && (
                <motion.div 
                  ...
                  className={`absolute left-0 top-0 bottom-0 w-1.5 bg-${tab.color}-500`} 
                />
              )}
              ...
              <div className="flex-1">
                 <p className={`text-xs font-black uppercase tracking-tight ${activeTab === tab.id ? 'text-white' : 'text-slate-400'}`}>
                    {tab.label}
                 </p>
                 <p className="text-[9px] font-bold text-slate-600 line-clamp-1">{tab.desc}</p>
              </div>

              {activeTab === tab.id && <ChevronRight className="w-4 h-4 text-slate-500" />}
            </button>
          ))}
        </nav>

        <div className="mt-auto p-6 bg-[#0f172a] rounded-[2rem] border border-white/5">
      ...
      <main className="flex-1">
        <AnimatePresence mode="wait">
          {activeTab === 'core' && (
            ...
            >
               {/* Scheduler Matrix */}
               <div className="bg-[#0f172a] border border-white/5 rounded-[3rem] p-10 space-y-8">
                  <div className="flex items-center justify-between">
                     <div className="space-y-1">
                        <h3 className="text-2xl font-black text-white uppercase tracking-tighter">Scheduler Matrix</h3>
                        <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Global Delivery Windows</p>
                     </div>
               ...
               {/* Thresholds Slider */}
               <div className="bg-[#0f172a] border border-white/5 rounded-[3rem] p-10 flex flex-col md:flex-row items-center gap-10">
                  <div className="flex-1 space-y-6 w-full">
                     <div className="flex justify-between items-end">
                        <div className="space-y-1">
                           <h4 className="text-lg font-black text-white uppercase">Data Threshold</h4>
                           <p className="text-[9px] font-bold text-slate-500 uppercase">Max Daily Broadcasts</p>
                        </div>
                        <span className="text-4xl font-black text-emerald-500 tabular-nums">{getValue('MAX_DAILY_PUSHES')}</span>
                     </div>
                     <input 
                  ...
                  <div className="w-full md:w-48 p-6 bg-emerald-500/5 border border-emerald-500/10 rounded-3xl flex flex-col items-center gap-2">
                     <ShieldCheck className="w-8 h-8 text-emerald-500" />
                     <p className="text-[10px] font-black text-white uppercase text-center tracking-tight">Congestion Protocol Active</p>
                  </div>
               </div>
               ...
               <div className="grid grid-cols-1 gap-6">
                  ...
                  <div className="bg-[#0f172a] border border-white/5 rounded-[3rem] p-10">
                     <div className="flex items-center justify-between mb-10">
                        <div className="space-y-1">
                           <h3 className="text-2xl font-black text-white uppercase tracking-tighter">Delivery Channels</h3>
                           <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Toggle active notification streams</p>
                        </div>
                  ...
                  {/* Universal Test Card */}
                  <div className="bg-gradient-to-br from-blue-600/10 via-[#0f172a] to-emerald-500/5 border border-white/5 rounded-[3rem] p-12 relative overflow-hidden group">
                     <div className="absolute right-0 top-0 opacity-5 group-hover:opacity-20 transition-opacity">
                        <Activity className="w-64 h-64 -translate-y-12 translate-x-12" />
                     </div>
                     <div className="relative z-10 max-w-lg space-y-6">
                        <h3 className="text-3xl font-black text-white uppercase tracking-tighter">Universal Broadcast Test</h3>
                        <p className="text-sm text-slate-400 font-medium leading-relaxed">Execute a full-stack diagnostic pulse. The system will select a knowledge fragment and attempt transmission through all active notification layers simultaneously.</p>
                        <button 
                          onClick={executeBroadcastTest}
                          disabled={broadcastLoading}
                          className="px-10 py-5 bg-white text-black rounded-[2rem] text-xs font-black uppercase tracking-[0.2em] hover:bg-emerald-500 hover:text-black hover:scale-105 active:scale-95 transition-all shadow-2xl disabled:opacity-30"
                        >
                  ...
                  <div className="bg-[#0f172a] border border-white/5 rounded-[3rem] p-10">
                     <div className="flex items-center gap-3 mb-8">
                        ...
                        <h3 className="text-lg font-black text-white uppercase tracking-tight">Telegram Infrastructure</h3>
                     </div>
            ...
            >
               <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div 
                    ...
                    className={`md:col-span-1 p-10 rounded-[3rem] border cursor-pointer transition-all flex flex-col justify-between group h-full ${getValue('ENABLE_SSO') === 'true' ? 'bg-indigo-500/5 border-indigo-500/20' : 'bg-white/5 border-white/5 opacity-50'}`}
                  >
                     <div className="flex items-center justify-between">
                        <div className={`p-6 rounded-3xl ${getValue('ENABLE_SSO') === 'true' ? 'bg-indigo-500 text-black shadow-xl shadow-indigo-500/30' : 'bg-slate-800 text-slate-500'}`}>
                           <Key className="w-10 h-10" />
                        </div>
                        <div className={`w-14 h-7 rounded-full p-1 transition-colors ${getValue('ENABLE_SSO') === 'true' ? 'bg-indigo-500' : 'bg-slate-700'}`}>
                           <div className={`w-5 h-5 rounded-full bg-white transition-transform ${getValue('ENABLE_SSO') === 'true' ? 'translate-x-7' : 'translate-x-0'}`} />
                        </div>
                     </div>
                     <div className="space-y-2">
                        <h3 className="text-3xl font-black text-white uppercase tracking-tighter">Central Auth</h3>
                  ...
                  {/* SSO Configuration */}
                  <div className="md:col-span-2 bg-[#0f172a] border border-white/5 rounded-[3rem] p-10 space-y-10">
                     <div className="grid grid-cols-1 gap-6">
                     ...
                     <div className="pt-10 border-t border-white/5">
                        ...
                        >
                          ...
                          {ssoTestLoading ? 'Handshaking with Identity Core...' : 'Verify CentralAuth Ecosystem Link'}
                        </button>
      ...
      <AnimatePresence>
        {broadcastResult && (
          <motion.div 
            ...
            className="fixed bottom-10 right-10 w-full max-w-sm z-[100]"
          >
             <div className="bg-[#1e293b]/95 backdrop-blur-2xl border border-emerald-500/30 rounded-[2.5rem] p-8 shadow-[0_30px_100px_rgba(0,0,0,0.5)]">
                <div className="flex items-center justify-between mb-8">
                   <div className="flex items-center gap-3">
                      <div className="p-3 bg-emerald-500 rounded-2xl shadow-lg shadow-emerald-500/40"><Activity className="w-5 h-5 text-black" /></div>
                      <h4 className="text-[11px] font-black uppercase text-white tracking-widest">Diagnostic Report</h4>
                   </div>
                ...
                <div className="space-y-4">
                   <div className="bg-black/40 p-5 rounded-3xl border border-white/5 flex justify-between items-center">
                      ...
                      <p className="text-xs font-black text-emerald-400 uppercase">{broadcastResult.telegram}</p>
                   </div>
                   <div className="bg-black/40 p-5 rounded-3xl border border-white/5 flex justify-between items-center">
                      <p className="text-[10px] font-black text-slate-500 uppercase">Chrome</p>
                      <p className="text-xs font-black text-blue-400 uppercase">{broadcastResult.web_push}</p>
                   </div>
                </div>
             </div>
          </motion.div>
