...
export default function Layout() {
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-slate-200 selection:bg-emerald-500/30 flex">
      ...
      {/* MAIN WRAPPER */}
      <div className="flex-1 md:ml-24 flex flex-col min-w-0">
           ...
           <div className="px-4 md:px-12 py-0.5 flex items-center gap-2 overflow-hidden border-t border-white/[0.03] bg-black/10">
              ...
              <div className="flex-1 flex items-center gap-2 overflow-x-auto no-scrollbar py-2">
               ...
               {tagsData?.slice(0, 50).map((t: any) => (
                  <button 
                    ...
                    className={`px-5 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap border group flex items-center gap-2 ${selectedTag === t.name ? 'bg-emerald-500 text-black border-emerald-400 shadow-[0_0_20px_rgba(16,185,129,0.3)]' : 'bg-white/5 text-slate-500 border-white/5 hover:border-emerald-500/20 hover:text-emerald-500'}`}
                  >
                     #{t.name}
                  </button>
               ))}
              ...
              <div className="pl-6 border-l border-white/10 hidden lg:flex items-center gap-4">
                 <button 
                   ...
                   className={`px-5 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap border ${showArchived ? 'bg-amber-500 text-black border-amber-400 shadow-[0_0_15px_rgba(245,158,11,0.3)]' : 'bg-white/5 text-slate-500 border-white/5 hover:text-amber-500'}`}
                 >
                    <Zap className="w-3.5 h-3.5 inline mr-2" /> Vault
                 </button>
              </div>
           </div>
        </header>
